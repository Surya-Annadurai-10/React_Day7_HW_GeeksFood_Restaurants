...
import './App.css'
import Nav from './Components/Nav/NavSection.jsx'
...
function App() {
  ...
  let searchVal = data.filter((ele) =>{
    return rate ?(( ele.name.toLowerCase()).includes(inputVal) ||
    ( ele.address.toLowerCase()).includes(inputVal) || 
    ( ele["address line 2"].toLowerCase()).includes(inputVal) ||
    ( ele.outcode.toLowerCase()).includes(inputVal) ||
    ( ele.postcode.toLowerCase()).includes(inputVal) ||
    ( ele.type_of_food.toLowerCase()).includes(inputVal)) && 
    (Math.floor( ele.rating) == (rate))

    :
      
      ( ele.name.toLowerCase()).includes(inputVal) ||
    ( ele.address.toLowerCase()).includes(inputVal) || 
    ( ele["address line 2"].toLowerCase()).includes(inputVal) ||
    ( ele.outcode.toLowerCase()).includes(inputVal) ||
    ( ele.postcode.toLowerCase()).includes(inputVal) ||
  ...
  return (
    <>
    <Nav />
    <Search rate={rate} setRate={setRate} inputVal = {inputVal} setInputVal ={setInputVal} />
    <Body data={searchVal} />
    </>
  )
}
